...
sheet <- read_sheet(
  "https://docs.google.com/spreadsheets/d/1K7VRu80v6qy1qhHgJqL9MrtPOsLaZbtBiMPRwuxgf9c/edit?usp=sharing",
                  col_names = TRUE,
...
```{r food_waste_rate_preprocessinbg}
## Calculate daily food waste rate ---------------------------------------------
# formula: daily food waste rate = all food waste / daily input food
# all food waste = daily solid and liquid food waste
# daily input food = one bowl weigh * (regular order + 1/2 mini orders)

ONE_BOWL_KG = 0.9
ONE_BOWL_SOUP_KG = 0.5

dailyInputKg  = ONE_BOWL_KG * (df$regularOrders + df$miniOrders / 2)
consumptionKg = dailyInputKg - allWasteKg
wasteRate = df$allWasteKg / dailyInputKg

inputLiquidKg = ONE_BOWL_SOUP_KG * (df$regularOrders + df$miniOrders / 2)

two_ma_foodLossKg <- numeric(length(foodLossKg)-1)
myMA <- function(arr,n=2){
  ma <- arr
...
two_ma_foodLossKg <- myMA(foodLossKg,2)
# for (t in 1:length(foodLossKg)-1){
#   temp <- numeric()
#   temp <- (foodLossKg[t] + foodLossKg[t+1]) / 2
#   two_ma_foodLossKg[t] <- ifelse(is.nan(temp), 0, temp)
# }
daily_hidden_loss_rate = liquidWasteKg[1:length(foodLossKg)] / two_ma_foodLossKg
```
...
# plot beta probability distribution daily food waste rate
plot(p, dbeta(p, allWasteKg[1], consumptionKg[1]), type='l')
# plot beta probability distribution daily food waste rate
plot(p, dbeta(p, sum(allWasteKg), sum(consumptionKg)), type='l')
```
...
week_day == "Thu", 1, -1) 
# Final data set ---------------------------------------------------------------
dt <- cbind(df,wasteRate,daily_hidden_loss_rate, week_day, wkMat, week_day_end)
```
...
```{r time_plot_waste_rate}
# Time Series Plot on all food waste rate --------------------------------------
tsWastePlot <- 
  ggplot() +
  geom_line(data = subset(dt, isClose %in% FALSE), 
            aes(x = date, y =wasteRate),
            color="dark blue") +
  # geom_point(data = dt, aes(x = date, y = daily_waste_rate, shape = isClose)) +
  scale_shape_manual(values=c(16, 4)) +
  theme(legend.position = c(0.1,0.15)) +
  # geom_rect(data = dt, aes(xmin = date, xmax = dplyr::lead(date), 
  #                          ymin = -Inf, ymax = Inf, 
  #                          fill = factor(!isClose)),  alpha = .3) +
  xlab("Date") +
  ylab("Daily Food Waste Rate") +
  ggtitle("Daily Food Waste Rate Trend")

tsHiddenLossPlot <- 
  ggplot() +
  geom_line(data = subset(dt, isClose %in% FALSE), 
            aes(x = date, y =daily_hidden_loss_rate),
            color="dark blue") +
  # geom_point(data = dt, aes(x = date, y = daily_waste_rate, shape = isClose)) +
  scale_shape_manual(values=c(16, 4)) +
  theme(legend.position = c(0.1,0.15)) +
  ...
  #                          fill = factor(!isClose)),  alpha = .3) +
  xlab("Date") +
  ylab("Daily Food Loss Rate") +
  ggtitle("Daily Hidden Food Loss Rate Trend")
# tsHiddenLossPlot

grid.arrange(tsWastePlot,tsHiddenLossPlot)
```
...
library(ggpubr)
ggqqplot(allWasteKg+foodLossKg)
ggqqplot(sqrt(allWasteKg+foodLossKg))
ggqqplot((allWasteKg+foodLossKg)^2)
ggqqplot(log(allWasteKg+foodLossKg+1))
shapiro.test(allWasteKg+foodLossKg)

ggqqplot(foodLossKg)
ggqqplot(sqrt(foodLossKg))
ggqqplot(log(foodLossKg+1))
shapiro.test(foodLossKg)

ggqqplot(allWasteKg)
ggqqplot(sqrt(allWasteKg))
ggqqplot(log(allWasteKg+1))
shapiro.test(allWasteKg)

ggqqplot(liquidWasteKg)
ggqqplot(sqrt(liquidWasteKg))
ggqqplot(log(liquidWasteKg+1))
shapiro.test(liquidWasteKg)

ggqqplot(solidWasteKg)
ggqqplot(sqrt(solidWasteKg))
ggqqplot(log(solidWasteKg+1))
shapiro.test(solidWasteKg)

library("dgof")
ks.test(allWasteKg+foodLossKg, "pnorm")
```


```{r histogram_all}
# Histogram on all food waste and loss -----------------------------------------
hist_all_LossWasteKg <- 
  ggplot(data = subset(dt, isClose %in% FALSE), aes(x = allWasteKg+foodLossKg)) +
  geom_histogram(aes(y = ..density..), bins = 30, colour = 1, fill = "white") +
  geom_density(linewidth = 1.5, colour = 4, fill = 4, alpha = 0.25) +
  labs(title = "Daily Food Loss and Waste - Histogram and Kernel Density Estimation")
hist_all_LossWasteKg
```

```{r histogram_all}
# Histogram on all food loss----------------------------------------------------
hist_all_LossKg <- 
  ggplot(data = subset(dt, isClose %in% FALSE), aes(x = foodLossKg)) +
  geom_histogram(aes(y = ..density..), bins = 30, colour = 1, fill = "white") +
  geom_density(linewidth = 1.5, colour = 4, fill = 4, alpha = 0.25) +
  labs(title = "Daily Food Loss - Histogram and Kernel Density Estimation")
hist_all_LossKg
```
...
hist_allWasteKg <- 
  ggplot(data = subset(dt, isClose %in% FALSE), aes(x = allWasteKg)) +
  geom_histogram(aes(y = ..density..), bins = 30, colour = 1, fill = "white") +
  geom_density(linewidth = 1.5, colour = 4, fill = 4, alpha = 0.25) +
  labs(title = "Daily Food Waste - Histogram and Kernel Density Estimation")
hist_allWasteKg

grid.arrange(hist_all_LossWasteKg,hist_all_LossKg,hist_allWasteKg)
# # Histogram on all food waste --------------------------------------------------
# hist_allWasteKg_dist <- 
#   ggplot(data = subset(dt, week_day_end %in% 1 ), aes(x = allWasteKg)) +
#   geom_histogram(aes(y = ..density..), bins = 30, colour = 1, fill = "white") +
#   # geom_density(linewidth = 1.5, colour = 4, fill = 4, alpha = 0.25) +
#   labs(title = "Daily Food Waste - Histogram and Kernel Density Estimation")
# hist_allWasteKg_dist
...
hist_liquidWasteKg <- 
  ggplot(data = subset(dt, isClose %in% FALSE), aes(x = liquidWasteKg)) +
...
hist_log_liquid_waste <- 
  ggplot(data = subset(dt, isClose %in% FALSE), aes(x = log(liquidWasteKg))) +
  geom_histogram(aes(y = ..density..), bins = 30, colour = 1, fill = "orange") +
...
# Log +1  transformed Histogram
hist_log_liquid_waste <- 
  ggplot(data = subset(dt, isClose %in% FALSE), aes(x = log(liquidWasteKg+1))) +
  geom_histogram(aes(y = ..density..), bins = 30, colour = 1, fill = "orange") +
  # labs(title = "Logged Daily Liquid Food Waste - Histogram")
  geom_density(data = dt,mapping = aes(x = log(liquidWasteKg+1)),
               size = 1.5, colour = 4, fill = 4, alpha = 0.25) +
  labs(title = "Logged Daily Liquid Food Waste - Histogram and Kernel Density Estimation")
hist_log_liquid_waste
...
hist_solidWasteKg <- 
  ggplot(data = subset(dt, isClose %in% FALSE), aes(x = solidWasteKg)) +
...
# Histogram of logged solid waste -----------------------------------------------------
hist_logged_solidWasteKg <- 
  ggplot(data = subset(dt, isClose %in% FALSE), aes(x = log(solidWasteKg))) +
  geom_histogram(aes(y = ..density..), bins = 30, colour = 1, fill = "blue") +
  geom_density(data = dt, mapping = aes(x = log(solidWasteKg)), 
  size = 1.5, colour = 4, fill = 4, alpha = 0.25) +
  labs(title = "Daily Solid Food Waste - Histogram and Kernel Density Estimation")
hist_logged_solidWasteKg

# Histogram of logged solid waste (log+1) -----------------------------------------------------
hist_logged_plus1_solidWasteKg <- 
  ggplot(data = subset(dt, isClose %in% FALSE), aes(x = log(solidWasteKg+1))) +
  geom_histogram(aes(y = ..density..), bins = 30, colour = 1, fill = "blue") +
  geom_density(data = dt, mapping = aes(x = log(solidWasteKg+1)), 
  size = 1.5, colour = 4, fill = 4, alpha = 0.25) +
  labs(title = "Daily Solid Food Waste - Histogram and Kernel Density Estimation")
hist_logged_plus1_solidWasteKg
```
...
ggplot(data = subset(dt,isClose %in% FALSE), 
       aes(x = allWasteKg, y = ..density.., fill=as.factor(week_day_end))) +
  geom_histogram(position = "identity", alpha=0.2,bins = 20) +
  geom_density(mapping = aes(x = allWasteKg), alpha=0.4)
```

```{r}
# Compare All Food Loss in Weekday and Weekend----------------------------------
# F test (equal var)
var.test(dt$foodLossKg[week_day_end==1], 
         dt$foodLossKg[week_day_end==-1],alternative = "two.sided")
# Welch t test
t.test(dt$foodLossKg[week_day_end==1], 
       dt$foodLossKg[week_day_end==-1], var.equal = FALSE)

## Compare All Food Waste in Weekday and Weekend--------------------------------
# F test (equal var)
var.test(dt$allWasteKg[week_day_end==1], 
         dt$allWasteKg[week_day_end==-1],alternative = "two.sided")
# Welch t test
t.test(dt$allWasteKg[week_day_end==1], 
       dt$allWasteKg[week_day_end==-1], var.equal = FALSE)

## Compare liquid Food Waste in Weekday and Weekend-----------------------------
# F test (equal var)
var.test(dt$liquidWasteKg[week_day_end==1], 
         dt$liquidWasteKg[week_day_end==-1],alternative = "two.sided")
# Welch t test
t.test(dt$liquidWasteKg[week_day_end==1], 
       dt$liquidWasteKg[week_day_end==-1], var.equal = FALSE)

## Compare solid Food Waste in Weekday and Weekend------------------------------
# F test (equal var)
var.test(dt$solidWasteKg[week_day_end==1], 
         dt$solidWasteKg[week_day_end==-1],alternative = "two.sided")
# Welch t test
t.test(dt$solidWasteKg[week_day_end==1], 
       dt$solidWasteKg[week_day_end==-1], var.equal = TRUE)
```
...
str(dt)
corr_ind <- round(cor(dt[c("foodLossKg",
                       "allWasteKg",
                       "liquidWasteKg",
                       "solidWasteKg")]), 
              2)
...
corr_ind_pValues <- round(cor_pmat(dt[c("foodLossKg",
                                    "allWasteKg",
                                    "liquidWasteKg",
                                    "solidWasteKg")]),4)
corr_ind_pValues

ggcorrplot(corr_ind, type = "lower", lab = TRUE)
```
...
corr_exp
# correlation p-values ---------------------------------------------------------
corr_exp_pValues <- round(cor_pmat(dt[c("tempC",
                                       "humidityPerc",
                                       "precipMM",
                                       "tue",
                                       "wed",
...
ggcorrplot(corr_exp, type = "lower", lab = TRUE)
```
...
all_thpsclw <- allWasteKg ~ tempC + humidityPerc + precipMM + week_day+ sales + customer + liquors 
all_thpcw   <- allWasteKg ~ tempC + humidityPerc + precipMM + week_day+ customer
all_thpsw   <- allWasteKg ~ tempC + humidityPerc + precipMM + week_day+ sales
all_thplw   <- allWasteKg ~ tempC + humidityPerc + precipMM + week_day+ liquors
all_thpclw  <- allWasteKg ~ tempC + humidityPerc + precipMM + week_day+ customer + liquors
all_thpslw  <- allWasteKg ~ tempC + humidityPerc + precipMM + week_day+ sales + liquors
all_thpscw  <- allWasteKg ~ tempC + humidityPerc + precipMM + week_day+ sales + customer
all_thpw    <- allWasteKg ~ tempC + humidityPerc + precipMM + week_day

all_mod <- lm(all_thpsclw,data = dt)
all_thpsclw_mods <- lm(formula = all_thpsclw, data = dt)
all_thpcw_mods   <- lm(formula = all_thpcw,   data = dt)
all_thpsw_mods   <- lm(formula = all_thpsw,   data = dt)
all_thplw_mods   <- lm(formula = all_thplw,   data = dt)
all_thpclw_mods  <- lm(formula = all_thpclw,  data = dt)
all_thpslw_mods  <- lm(formula = all_thpslw,  data = dt)
all_thpscw_mods  <- lm(formula = all_thpscw,  data = dt)
all_thpw_mods    <- lm(formula = all_thpw,    data = dt)
summary(all_mod)
summary(all_thpsclw_mods)
summary(all_thpcw_mods)
summary(all_thpsw_mods)
summary(all_thplw_mods)
summary(all_thpclw_mods)
summary(all_thpslw_mods)
summary(all_thpscw_mods)
summary(all_thpw_mods)
```
...
p_a <- plotSSM(mcmc_sample = mcmc_sample, 
               time_vec = dt$date, 
               obs_vec = dt$allWasteKg,
               state_name <- mcmc_sample$alpha, 
               graph_title = "Estimation of Level", 
               y_label = "All Food Waste in Kg") 

p_b1 <- plotSSM(mcmc_sample = mcmc_sample, 
               time_vec = dt$date,
               state_name = mcmc_sample$beta_temp, 
               graph_title = "Estimation of change in temperature effect", 
               y_label = "coef temp") 

p_b2 <- plotSSM(mcmc_sample = mcmc_sample, 
               time_vec = dt$date,
               state_name = mcmc_sample$beta_humi, 
               graph_title = "Estimation of change in humidity effect", 
               y_label = "coef humidity") 

grid.arrange(p_all, p_a, p_b1, p_b2)
