...
par(mfrow = c(2, 2)); par(oma = c(0, 0, 0, 0)); par(mar = c(4, 4, 2, 1))
# (a) ナイル川における年間流量
hist(Nile, main = "(a)", xlab = "Annual Precipitation")
summary(Nile)
# (b) 大気中の二酸化炭素濃度
hist(y_CO2, main = "(b)", xlab = "CO2")
summary(y_CO2)
# (c) 英国における四半期毎のガス消費量
hist(UKgas_log, main = "(c)", xlab = "Quatally Gas Consumption")
summary(UKgas_log)
# (d) 非線形なモデルから生成した人工的なデータ
hist(y_nonlinear, main = "(d)", xlab = "Simulation")
summary(y_nonlinear)
# 描画に関する後処理
par(oldpar)
```
...
# プロット
ts.plot(temperature_with_outl, ylim = c(20, 40), type = "l", ylab = "F Temp")
# ヒストグラムと五数要約
hist(temperature_with_outl, breaks = seq(from = 20, to = 80, by = 1), 
     xlab = "C Temp", ylab = "Frequency", main = "")
...
plot.spectrum <- function(dat, lab = "", main = "",
  ...
  plot(dat_FFT/max(dat_FFT), type = "l", main = main,
       ylab = "Norm. Freq. Spectrum", ylim = c(0, y_max),
       xlab = sprintf("Freq.[1/%s]", lab), xlim = c(1, data_len/2), xaxt = "n")
...
# (d) 非線形なモデルから生成した人工的なデータ
plot.spectrum(y_nonlinear, lab = "Time", main = "(d)")
# 以降のコードは表示を省略
# 縦軸のスケールを変えて再描画
# (a) ナイル川における年間流量
...
plot.spectrum(  UKgas_log, lab =   "Month", main = "(c)", y_max = 0.2 ,
              tick = c(12, 6), unit = 3)
# (d) 非線形なモデルから生成した人工的なデータ
plot.spectrum(y_nonlinear, lab = "Time", main = "(d)", y_max = 0.2 )
# 描画に関する後処理
par(oldpar)
```
...
mtext(text = "Level", side = 1, line = 1.5)
...
mtext(text = "Trend", side = 1, line = 1.5)
mtext(text = "Time", side = 1, at = 10*pi, line = 0.3, cex = 0.7)
plot.new()  # ダミーパネル
# 周期成分
plot.new()  # ダミーパネル
