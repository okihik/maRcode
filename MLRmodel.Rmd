...
# Set Restaurant Open Data only ------------------------------------------------
df_open <- data.frame(
  # Obs. date and the restaurant close or not
  date    = as.Date(sheet$date[!dt$isClose]),
  isClose = as.logical(sheet$isClose[!dt$isClose]),
  # food loss and waste at the restaurant
  foodLossKg    = foodLossKg[!dt$isClose],
  allWasteKg    = allWasteKg[!dt$isClose],
  liquidWasteKg = liquidWasteKg[!dt$isClose],
  solidWasteKg  = solidWasteKg[!dt$isClose],
  # types of orders
  regularOrders = as.numeric(sheet$orders[!dt$isClose]),
  miniOrders    = as.numeric(sheet$miniOrders[!dt$isClose]),
  takeouts      = as.numeric(sheet$takeouts[!dt$isClose]),
  # 
  customer = as.numeric(sheet$customers[!dt$isClose]),
  liquors  = as.numeric(sheet$liquors[!dt$isClose]),
  sales    = as.numeric(sheet$sales[!dt$isClose]),
  # weather conditions
  tempC        = as.numeric(sheet$hrTM[!dt$isClose]),
  humidityPerc = as.numeric(sheet$hrHM[!dt$isClose]),
  precipMM     = as.numeric(sheet$precip[!dt$isClose])#,
)
...
```{r data_liquid_solid_descriptive}
...
# Correlation b/w Dep. vars.
```{r correlation_ind.vars}
# correlation between dependent variables ------------------------------------
# str(dt)
corrDep <- round(cor(dt[c("foodLossKg",
                       "allWasteKg",
                       "liquidWasteKg",
                       "solidWasteKg")]), 
              2)
...
corrDep_pValues <- round(cor_pmat(dt[c("foodLossKg",
                                    "allWasteKg",
                                    "liquidWasteKg",
                                    "solidWasteKg")]),4)
corrDep_pValues

ggcorrplot(corrDep, type = "lower", lab = TRUE)
```
...
corrExp
# correlation p-values ---------------------------------------------------------
corrExp_pValues <- round(cor_pmat(dt[c("tempC",
                                       "humidityPerc",
                                       "precipMM",
                                       "tue",
                                       "wed",
...
ggcorrplot(corrExp, type = "lower", lab = TRUE)
```
