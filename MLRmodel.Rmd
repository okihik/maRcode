...
if(!(all((foodLossKg >= 0)|
         ...
         (liquidWasteKg >= 0)|
         (solidWasteKg >= 0) == TRUE)))
    print("The date has negative values.")
...
# basic summary for restaurant open --------------------------------------------
...
tsPlot <- 
  ...
  geom_line(data = dt, aes(x = date, y = liquidWasteKg), 
...
# # Log transformed Histogram
# hist_log_liquid_waste <- ggplot(data = dt, aes(x = log(liquidWasteKg))) +
# geom_histogram(aes(y = ..density..), colour = 1, fill = "orange", bins = 30) +
# labs(title = "Logged Daily Liquid Food Waste - Histogram")
# hist_log_liquid_waste
# 
# kernel_hist_log_liquid_waste <- hist_log_liquid_waste +
# geom_density(data = dt, 
# mapping = aes(x = log(liquidWasteKg)), 
# size = 1.5, colour = 4, fill = 4, alpha = 0.25) +
...
ccf(dt["foodLossKg"],dt["allWasteKg"])
ccf(dt["foodLossKg"],dt["liquidWasteKg"])
ccf(dt["foodLossKg"],dt["solidWasteKg"])
ccf(dt["allWasteKg"],dt["liquidWasteKg"])
ccf(dt["allWasteKg"],dt["solidWasteKg"])
ccf(dt["liquidWasteKg"],dt["solidWasteKg"])
```
...
# correlation between independent variables ------------------------------------
#str(dt)
corr <- round(cor(dt[c()]),2)
corr
# correlation p-values ---------------------------------------------------------
corr_pValues <- round(cor_pmat(dt[c(11:12,16:18)]),4)
corr_pValues
```
...
mcmc_acf_bar(mcmc_sample, pars = c("s_u", "s_v", "s_w", "lp__"))
mcmc_trace(mcmc_sample, pars = c("s_u", "s_v", "s_w", "lp__"))
