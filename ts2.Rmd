...
#【平均と分散の例】
rate <- 1; shape <- 2
curve(dgamma(x, rate = rate, shape = shape), 0, 10, xlab = "Realizations", ylab = "Density")
g_mean <- rate * shape
g_sd   <- sqrt(rate * shape^2)
lines(c(g_mean, g_mean),
      c(0, dgamma(x = g_mean, rate = rate, shape = shape)),
      lty = "dotdash")
...
#【正規分布の例】
# 一次元
curve(dnorm, -3,  3, xlab = "Realiztions", ylab = "Density")
# 二次元
# 前処理
library(mvtnorm)
...
acf(y, main = "", xlab = "Lag", ylab = "Autocorration")
# 規則的な周期が存在する場合
rad_seq <- seq(from = 0, to = 2*pi*8, length.out = 100)
y <- ts(cos(rad_seq), freq = 2*pi)
plot(x = rad_seq/(2*pi), y = y, type = "l", xlab = "Time [period]", ylab = "Data");
acf(y, main = "", xlab = "Lag [periods]", ylab = "Autocorration")
# 後処理
par(oldpar)
```
